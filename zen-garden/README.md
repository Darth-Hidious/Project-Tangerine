...
| `experiments/` | `poc_run.py` (the whole garden, end to end), `arm_study.py` (which arm, which drives), `comb_study.py` (pattern variety), `erase_study.py` (sand over many cycles), `export_3d.py` (the 3D viewer) |
...
- **Landscape.**
  ...
  - The frame is one height all round. At every wall the ground stays 5 mm below its rim. The negative control, a 60 mm frame, leaves the spring's bank 12 mm short.
