...
| `water.py` | Evaporation, pump operating point, pipe losses, stream film depth |
| `viz.py` | Plan-view drawings |
| `cli.py` | `karesansui plan` and `karesansui check` |
| `experiments/` | `poc_run.py` (the whole garden, end to end), `arm_study.py` (which arm, which drives), `comb_study.py` (pattern variety), `erase_study.py` (sand over many cycles) |
...
- **Planning.**
  ...
  - Anything taller than the travel lift and within reach, the bonsai included, is reported.
- **Kinematics.**
  - IK inverts FK.
