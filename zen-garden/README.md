...
stream, a bonsai, two lanterns) with a bed of fine white sand that an arm built into the garden
...
- **Bonsai (the tree the pictures draw).**
  - Every needle stays inside the canopy outline the arm keeps clear of, and under the tree's height.
...
- **Network access to `colab.research.google.com`.** In a Claude Code cloud environment, add it to the allowed domains.
